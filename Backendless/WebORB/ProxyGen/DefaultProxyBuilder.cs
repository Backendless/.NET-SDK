...
namespace Weborb.ProxyGen.DynamicProxy
{
	...
	public class DefaultProxyBuilder : IProxyBuilder
	{
		...
		{
			bool isNestedAndInternal = (target.IsNestedPublic || target.IsNestedPrivate) && (target.IsNestedAssembly || target.IsNestedFamORAssem);
			bool isInternalNotNested = target.IsVisible == false && (target.IsNestedPublic == false || target.IsNestedPrivate == false) ;
