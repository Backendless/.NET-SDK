			...
			if (argumentNames.Length != 0)
			{
				...
				for(int i = 0; i < genericTypeParams.Length; i++)
				{
					try
					{
                        ...
                        List<Type> interfacesConstraintsList = new List<Type>();

                        foreach( Type t in types )
                            if( t.IsInterface )
                                interfacesConstraintsList.Add( t );

                        Type[] interfacesConstraints = interfacesConstraintsList.ToArray();
