...
namespace Weborb.ProxyGen.DynamicProxy.Generators
{
	using System;
	...
	using Weborb.ProxyGen.DynamicProxy.Generators.Emitters;
	using Weborb.ProxyGen.DynamicProxy.Generators.Emitters.CodeBuilders;
	using Weborb.ProxyGen.DynamicProxy.Generators.Emitters.SimpleAST;
	using Weborb.ProxyGen.DynamicProxy.Serialization;
	...
	public abstract class BaseProxyGenerator
	{
        ...
        private IDictionary<MethodInfo, FieldReference> method2TokenField = new Dictionary<MethodInfo, FieldReference>();
        private List<MethodInfo> generateNewSlot = new List<MethodInfo>();
        protected List<MethodInfo> methodsToSkip = new List<MethodInfo>();
		                                           ...
		                                           LocalReference invocationImplLocal)
		{
            ...
            foreach( Type t in genArgs )
                if( t.IsGenericParameter )
                    genericParametersList.Add( t );

            Type[] genericParameters = genericParametersList.ToArray();
			...
			MethodInfo callbackMethod)
		{
			...
			foreach( KeyValuePair<int, LocalReference> keyvalue in byRefArguments)
			{
                int index = keyvalue.Key;
				LocalReference localReference = keyvalue.Value;
				method.CodeBuilder.AddStatement(
		...
		protected void CollectMethodsToProxy(IList methodList, Type type, bool onlyVirtuals)
		{
			CollectMethods(methodList, type, onlyVirtuals);

			if (type.IsInterface)
			{
				Type[] typeChain = type.GetInterfaces();// .FindInterfaces(new TypeFilter(NoFilter), null);
		...
		protected void CollectPropertyMethodsToProxy(
			IList methodList, Type type, bool onlyVirtuals, ClassEmitter emitter, out PropertyToGenerate[] propsToGenerate)
		{
			if (type.IsInterface)
			{
		...
		private void CollectEventMethodsToProxy(
			IList methodList, Type type, bool onlyVirtuals, ClassEmitter emitter, out EventToGenerate[] eventsToGenerates)
		{
			if (type.IsInterface)
			{
		...
		private void CollectMethods(IList methodsList, Type type, bool onlyVirtuals)
		{
		...
		private EventToGenerate[] CollectEvents(IList methodList, Type type, bool onlyVirtuals, ClassEmitter emitter)
		{
            List<EventToGenerate> toGenerateList = new List<EventToGenerate>();
		...
		private PropertyToGenerate[] CollectProperties(
			IList methodList, Type type, bool onlyVirtuals, ClassEmitter emitter)
		{
            List<PropertyToGenerate> toGenerateList = new List<PropertyToGenerate>();
