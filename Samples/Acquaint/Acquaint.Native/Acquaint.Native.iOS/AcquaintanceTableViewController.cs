  ...
  public partial class AcquaintanceTableViewController : UITableViewController, IUIViewControllerPreviewingDelegate
  {
    ...
    async Task RefreshAcquaintances()
    {
      ...
      catch( Exception ex )
      {
        ...
        using( var alert = new UIAlertView( "Error getting acquaintances", "Ensure you have a network connection, and that you entered valid Backendless Application ID and .NET API Key in the Acquaint.Util.NetStandard project (see the Settings class)", null, "OK" ) )
        {
          alert.Show();
        }
      }
