...
namespace Acquaint.XForms
{
  public class AcquaintanceDetailViewModel : BaseNavigationViewModel
  {
    ...
    public async Task<Position> GetPosition()
    {
      ...
      if( p.Latitude == 0 && p.Longitude == 0 && AddressBeginsWithNumber( Acquaintance.AddressString ) && Device.RuntimePlatform == Device.Android )
      {
    ...
    static string GetAddressWithRoundedStreetNumber( string address )
    {
      ...
      roundedNumber = (int) Math.Round( (decimal) originalNumber / 100, 0 ) * 100;//.RoundToLowestHundreds();
