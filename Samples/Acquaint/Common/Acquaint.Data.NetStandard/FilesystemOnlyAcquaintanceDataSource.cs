...
namespace Acquaint.Data
{
  ...
  public class FilesystemOnlyAcquaintanceDataSource : IDataSource<Acquaintance>
  {
    ...
    {
      _RootFolder = Environment.GetFolderPath( Environment.SpecialFolder.LocalApplicationData );
    }
    ...
    public async Task<Acquaintance> GetItem( string id )
    {
      ...
      return await Task.FromResult( _Acquaintances.SingleOrDefault( x => x.ObjectId == id ) ).ConfigureAwait( false );
    }
    ...
    public async Task<bool> UpdateItem( Acquaintance item )
    {
      ...
      int i = _Acquaintances.FindIndex( a => a.ObjectId == item.ObjectId );
    ...
    public async Task<bool> RemoveItem( Acquaintance item )
    {
      ...
      _Acquaintances.RemoveAll( c => c.ObjectId == item.ObjectId );
    ...
    {
      if( !FileExists( _RootFolder, _FileName ) )
      {
        CreateFile( _RootFolder, _FileName );
      }

      if( string.IsNullOrWhiteSpace( await GetFileContents( _RootFolder, _FileName ) ) )
      {
      ...
      {
        _Acquaintances = JsonConvert.DeserializeObject<List<Acquaintance>>( await GetFileContents( _RootFolder, _FileName ) );
      }
    ...
    static bool FileExists( string folder, string fileName )
    {
      return File.Exists( Path.Combine( folder, fileName ) );
    }

    static FileStream CreateFile( string folder, string fileName )
    {
      return File.Create( Path.Combine( folder, fileName ) );
    }

    static async Task WriteFile( string folder, string fileName, string fileContents )
    {
      await Task.Run( () =>
      {
        File.WriteAllText( Path.Combine( folder, fileName ), fileContents );
      } ).ConfigureAwait( false );
    }

    static async Task<string> GetFileContents( string folder, string fileName )
    {
